...
展示地址 ``/display/{项目名称}`` 是名称派生的，而名称可以被改（草稿里的 name 就是项目名）。
改完之后，已经配对好的墙面平板手里那份旧地址就永久 404 了，且没有任何提示 —— 用户只能把平板
拆下来重新配对（B38）。这张表记「旧名称 → 项目」，让旧地址继续可用（303 跳到当前地址）。

不做的取舍：不把已有项目的历史名称批量补进来 —— 改名发生在过去时，旧名称根本没有被记录下来
（这正是缺陷本身），任何「猜出旧名称」的回填都只能编造。因此这个迁移只建表，从这一刻起发生的
改名才会被记住。

降级同样只删表：别名是纯粹的兼容层，删掉它只是让旧地址重新 404，不影响任何项目、草稿或配对
设备的数据。
'''
