...
项目名只在应用层做过「先查后写」的唯一性校验，并发创建可以产生两行同名项目（B10），而名称同时是
展示地址 ``/display/{名称}`` 的路径段 —— 同名项目会让展示页定位不到。老库里可能已经有这种重名行
（正是这个缺陷的产物），直接建唯一索引会失败，因此先做一次确定性的改名：

按 ``(created_at, id)`` 排序，每组同名里最早的那一行保持原名，其余的追加「（2）」「（3）」…，
直到不与任何现存名称冲突。排序键固定，同一个库重复跑也得到同样的结果；改掉的只是重复的那几行，
唯一的原名仍然指向最早创建的项目。

改名是只能向前的一步：0002 的回退只删索引，不把名字还原 —— 「哪几行原本重名」这个信息在改名后
已经不存在了，硬要回退只能猜。
'''
