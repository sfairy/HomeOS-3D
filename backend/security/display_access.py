...
def resolve_display_project(database: Session, project_name: str) -> tuple[Project | None, str | None]:
    ...
    该指向它）。没有命中时再查改名留下的别名 —— 这就是改名之后，已经配对、
...
def active_display_device(
    ...
    有效期判定刻意收在这里，而不是留给调用方：判定若只写在 HTTP 依赖里，
    展示页路由与实时连接握手各自查了一遍库却都没查有效期，于是这两条路完全绕过了
    display_token_ttl_seconds / display_token_hard_ttl_seconds。``settings`` 因此是必填参数：
    新调用方没有办法「忘了传」而悄悄退回旧行为，漏传会立刻 TypeError。

    过期设备只判「不该放行」，不删行 —— 管理员列表里还要能看到它并手动解绑。

    配对码一项的口径：停用某个配对码即可让这一批设备同时失效，而没有关联配对码的历史设备
    依然有效，直到管理员显式吊销。
    """
