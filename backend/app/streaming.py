...
两个上传端点（3D 导出 ZIP、用户素材图片）形状一样：``async for chunk in request.stream()``
边收边写，同时逐块累计一个字节上限。风险也一样 —— 写盘、flush、fsync 都是同步调用，直接写在
``async def`` 路由里，一次大上传就会让所有 HTTP 与 WebSocket 一起等（B5 / B6）。

这里不引入异步文件库，只做两件事：把「收流 → 攒批 → 写盘」收敛成一处，顺便给待写数据一个
明确的内存上界；把必须同步完成的 fsync 单独包一层，让调用方一眼看出「这段要放进线程池」。

上限判定不在这里做：两个端点的状态码与中文文案都不同（413 的文案要带各自的 MB 数），因此以
``before_write`` 回调交回调用方，抛异常即中断。
"""
