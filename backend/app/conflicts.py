...
def is_unique_violation(error: IntegrityError, column: str) -> bool:
    ...
    - ``UNIQUE``：只按列名判断的话，**非空约束**的消息里同样会出现列名
      （``NOT NULL constraint failed: projects.name``）。于是「name 恒为 NULL」
      这种缺陷会被当成「重名」报给用户 —— 真正的 bug 藏在一句听起来很合理的
      提示后面，而且按「重名」重试再多次也是同样的失败。
    - ``column``：必须带表名（``projects.name`` 而不是 ``name``）。整条消息里
      表名与列名都在，只写列名的话，``display_pairing_codes.name`` 之类别的表上
      的同名列也会命中。

    参数:
        error: 捕获到的 ``IntegrityError``。
        column: 带表名的限定列名，例如 ``projects.name``。

    返回:
        确实是这一列上的唯一约束冲突则为 True。
    """
