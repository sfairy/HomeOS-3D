...
户型快照是「冻结一次、长期可读」的不可变文件：一份 scene JSON 加上随它冻结的
底图副本（可能几 MB）。冻结入口只有 ``POST /scenes`` 一条，而**此前没有任何删除
路径** —— 项目删除、场景替换、反复冻结算下来只在加文件，磁盘只增不减。

这里补上回收，规则刻意保守（删掉仍在用的快照 ＝ 看板黑屏，比多占几 MB 严重得多）：

1. **归属判定**：一个快照「还有没有人用」由**项目草稿文档里有没有出现这个 sceneId**
   决定，与 ``require_scene_viewer`` 的可见性判定同源；
2. **巡检**：``sweep_scenes`` 只回收「没有被任何草稿引用」**且**超过
   ``SCENE_TTL_SECONDS`` 的快照。被引用的快照无论多老都不动；
3. **触发时机**：冻结新快照时、删除项目时各跑一次。都是低频操作，不会给自动保存
   这类热路径加成本；
4. **残渣**：``delete_scene_files`` 只做到「尽力而为」，单文件删不掉时会留下
   「JSON 已没了、底图副本还在」的中间态。这类副本没有 owner 可查，因此按同一个
   「没人引用 + 已过保留期」的规则单独收一遍（见 :func:`orphan_scene_copies`）。

为什么不落一份索引文件：索引会与实际文件漂移，而这里需要的三件事（有哪些快照、
多大、多久没动）都能直接从文件系统读出来 —— 快照文件只在创建时写一次，读取不更新
mtime，所以 ``mtime`` 就是冻结时刻。少一份需要同步的状态，就少一处「索引说还在、
文件其实已经没了」的故障。

为什么不「项目删除就立刻删它的快照」：sceneId 可能只存在于某个浏览器本地状态里
（用户刚冻结、还没保存进仪表盘），立刻删会让那次编辑莫名丢失。统一等 TTL，
代价只是多占一段时间磁盘。
"""
...
def sweep_scenes(
    ...
    *,
    ...
    参数:
        folder: 快照目录；不存在时返回全零统计。
        referenced: 仍被引用的 sceneId 集合（见 :func:`referenced_scene_ids`）。
        now: 当前时间戳（秒，便于测试注入）；默认取系统时间。
        ttl_seconds: 未被引用的快照保留多久。

    返回:
        ``{'deleted': 删除的快照数, 'released': 释放的字节数, 'kept': 保留的快照数}``。

    除完整快照外，还会清掉「主人已经不在了」的底图副本残渣（见
    :func:`orphan_scene_copies`）—— ``delete_scene_files`` 只做到「尽力而为」，
    部分失败会留下这类残渣，而只按 ``*.json`` 巡检的话它谁也看不见。
    """
