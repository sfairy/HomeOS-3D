...
class BoundedAttemptLimiter:
    ...
    为什么需要这一层：``LoginAttemptLimiter`` 的键由调用方给定，一旦键来自外部输入
    （被尝试的配对码、被猜的用户名），键空间就等于外部随便造 —— 不封顶就是一条内存放大
    路径，也是 B33 记的那个问题。

    策略：按最近使用顺序记住键，超过 ``max_keys`` 就淘汰最久未用的（连它的失败与封禁记录
    一起清掉）。代价是「被淘汰的键等于重新开始计数」—— 想挤掉自己正在磨的那个键，得先造出
    ``max_keys`` 个其它键，每一次都要花掉调用方那一层的共享预算（配对码的跨来源预算
    30 次/分钟，见 ``api/displays.py``），因此比正常记账贵得多。这一层只用来兜底，
    不能替代共享预算。
    """
...
class LoginAttemptLimiter:
    ...
    def _trim(self, now: float, *, keep_key: str | None = None) -> None:
        ...
        为什么需要这一步：键来自外部（被猜的用户名、被试的配对码、对端地址），而清理**只发生
        在同一个键被再次查询时** —— 攻击者只要一直换新键，这些记录就再也不会被访问、也就永远
        不会被清掉，内存只涨不落（B33）。
    ...
    def retry_after(self, key: str) -> int:
        ...
        与 ``block_seconds``（封禁**总**时长）不是一回事，不要混用（B63）：调用方拿它拼
        ``Retry-After`` 时指的是「从现在起还要等多久」。用总时长会一路偏乐观 —— 一个已经封了
        9 分钟的键，客户端会被要求再等满一个窗口（10 分钟），而它其实 1 分钟后就能再试。
        反过来读太快（在封禁第 9 分钟告诉客户端「等 0 秒」）会让客户端立刻重试又立刻被拒，
        所以这里向上取整、至少 1 秒。

        返回值随多次调用递减（基于 monotonic，不受系统时间校准影响）；顺手清理已到期的封禁标记。
        """
        now = monotonic()
        with self._lock:
