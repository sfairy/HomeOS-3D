...
def _dossier(*rows: tuple[str, str]) -> str:
    ...
    每组生成一行 `<div>`（`dl` 里用 div 分组是 HTML5 允许的写法），行自己的上边线
    来自 CSS —— 表体那几条细分隔线是「这四行在读，而不是在排版」的全部证据，
    所以不在这里拼任何样式。

    让调用方逐个手写那层 div 是个必然出错的设计：漏一个整张表的对齐就散一格，
    而它看起来只是某一行缩进怪了一点，没人会当 bug。
    """
    return ''.join(
...
#:
#: 分工（改文案前先读一遍，五处各说一件事，任何两句能互换位置就说明有一处该删）：
#:   TITLE          这一页在哪、过了它是什么（左栏最重的一块字）
#:   TAGLINE        这一步具体做什么（一句话）
#:   DOSSIER        这台机器 / 这道门的客观事实（四行键值，左栏的表）
#: 右栏的 h1 / desc 说「你要做什么」，开通轨说「走到哪」，甲板说「现在什么样」，
#: 坞脚的 .hos-panel__meta 说「这一页怎么运作」。
#:
#: 档案刻意只放**不会变的事实**（绑定方式、数据落点、凭证形态），不放此刻的读数 ——
#: 读数归甲板。放一个变量，左栏就会和右栏说同一句话，两栏一起变成废字。
#:
#: 还有一条硬规矩：**不许逐字重复坞脚那份 meta**。两栏在同一屏里并排站着，同一句话
#: 说两遍不会显得信息多，只会显得这页没有别的可说。事实归档案（左栏是表，位置固定、
#: 可扫读），meta 只留贴在输入框旁边才有用的字段级提示（「6 位数字码」「邮箱最多 255
#: 字符」）。所以左上这几行里用过的句子，对应的页面 HTML 里那份 meta 就该少一条 ——
#: 本轮就是这么对齐的（login / pair / license-recovery 三页各删了一条或整行，
#: 商店登录屏删了「邮箱即账号」）。
#:
#: 「档案」与「坞脚的 <p>」「抬头 desc」之间允许语义相通但措辞不同：那是一句散文，
#: 这是同一件事的表格读法，两处各有各的用处。逐字撞车才是要避免的。
