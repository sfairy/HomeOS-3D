...
async function withBusy(form, task, busyText = '处理中…') {
...
const PENDING_FILTER_VALUES = new Map();
...
const ORDER_TYPE = { base: '基础', addon: '增购', upgrade: '升级', package: '套餐' };
const LICENSE_ACTION = { issue: '签发', patch: '补发', upgrade: '升级' };
...
function menuItem(label, attrs, { danger = false } = {}) {
...
function askConfirm({ title, message, impact = '', tone = 'danger', okText = '确认' }) {
...
function openAdjustDialog(accountId, email) {
...
$('#adjust-ok').addEventListener('click', async () => {
  ...
  try {
    ...
    settleAdjust(null);
    await Promise.all([loadAccounts(), loadLedger(), loadOverview()]);
...
// 修改密码弹窗
// --------------------------------------------------------------------------- //
const changePwDialog = $('#change-pw-dialog');
...
function jumpFromOverview(target) {
  ...
  const tab = collectTabs(target.page) ? 'list' : '';
  location.hash = tab ? `#${target.page}/${tab}` : `#${target.page}`;
  activate(target.page, tab);
}
...
async function loadOverview() {
  ...
  setNavBadge('orders', data.pendingOrders);
  setNavBadge('withdrawals', data.pendingWithdrawals);
}
...
function syncFeatureSummary(root) {
...
function renderFeatureOptions(root) {
...
function collectFeatureSelection(root) {
...
function closeFeaturePicker(root) {
...
function openFeaturePicker(root) {
  const pop = $('[data-feature-pop]', root);
  if (!pop) return;
  closeRowMenus();
  closeFeaturePickers();
...
$('#product-cancel').addEventListener('click', () => { hideEditor('#product-editor'); });
...
$('#product-rows').addEventListener('click', async (event) => {
  ...
  if (deleteId) {
    ...
    try {
      ...
      await Promise.all([loadProductCatalog(), loadProducts(), loadOverview()]);
    } catch (error) { toast(error.message, 'danger'); }
  }
});
...
$('#license-issue').addEventListener('click', () => {
  showEditor('#license-editor');
  $('#license-issue-result').hidden = true;
});
$('#license-cancel').addEventListener('click', () => { hideEditor('#license-editor'); });
...
$('#license-form').addEventListener('submit', async (event) => {
  ...
  try {
    await withBusy(form, async () => {
      ...
      resetPage('licenses');
      await Promise.all([loadLicenses(), loadOverview()]);
    }, '签发中…');
  } catch (error) { toast(error.message, 'danger'); }
});
...
$('#license-patch-cancel').addEventListener('click', () => { hideEditor('#license-patch-editor'); });
...
$('#license-patch-form').addEventListener('submit', async (event) => {
  ...
  try {
    await withBusy(form, async () => {
      ...
      hideEditor('#license-patch-editor');
      await loadLicenses();
    }, '保存中…');
  } catch (error) { toast(error.message, 'danger'); }
});
...
$('#license-rows').addEventListener('click', (event) => {
  ...
  showEditor('#license-patch-editor');
});
...
function openCouponEditor(coupon) {
  ...
  if (!coupon) { showEditor('#coupon-editor'); return; }
  ...
  showEditor('#coupon-editor');
}

$('#coupon-new').addEventListener('click', () => openCouponEditor(null));
$('#coupon-cancel').addEventListener('click', () => { hideEditor('#coupon-editor'); });
...
$('#coupon-rows').addEventListener('click', async (event) => {
  ...
  if (redemptionsFor) {
    redemptionFilter = { couponId: redemptionsFor, couponCode: event.target.dataset.couponCode || redemptionsFor };
    pageState('coupon-redemptions').offset = 0;
    // 落到「优惠码核销」那个 tab：核销表不在默认 tab 上，停在默认 tab 的话
    // 下面那句 scrollIntoView 会对着一个 hidden 元素做无效滚动。
    await activate('diagnostics', 'redeem');
...
function openAccountEditor(account) {
  ...
  showEditor('#account-editor');
  form.elements.email.focus();
}

$('#account-refresh').addEventListener('click', () => { resetPage('accounts'); loadAccounts(); });
$('#account-cancel').addEventListener('click', () => { hideEditor('#account-editor'); });
 ...
 */
function renderDiagnostics(selector, checks) {
  const list = $(selector);
  list.textContent = '';
...
function clearDiagnostics(selector) {
  const list = $(selector);
...
function selectedMailPreset(mail) {
 ...
 */
function syncMailPresetHint(mail) {
  const select = $('#mail-preset-provider');
 ...
 */
function applyMailPreset(preset, mail, { overwrite = true } = {}) {
...
function mailPasswordPayload(form) {
 ...
 */
function renderAlipayBadge(alipay) {
  const form = $('#settings-form');
...
function alipaySecretPayload(form) {
  const payload = {};
...
function openReleaseEditor(release) {
...
$('#release-new').addEventListener('click', () => {
  ...
  showEditor('#release-editor');
});

$('#release-cancel').addEventListener('click', () => hideEditor('#release-editor'));
...
$('#audit-purge').addEventListener('click', async () => {
  ...
  try {
    ...
    await Promise.all([loadAudits(), loadOverview()]);
  } catch (error) { toast(error.message, 'danger'); }
});
...
function openEntitlementEditor(entry) {
  ...
  renderFeatureOptions(picker);
  syncFeatureSummary(picker);
  showEditor('#entitlement-patch-editor');
}
...
$('#entitlement-new').addEventListener('click', () => {
  ...
  renderFeatureOptions(picker);
  syncFeatureSummary(picker);
  showEditor('#entitlement-editor');
});
$('#entitlement-cancel').addEventListener('click', () => { closeFeaturePickers(); hideEditor('#entitlement-editor'); });
$('#entitlement-patch-cancel').addEventListener('click', () => { closeFeaturePickers(); hideEditor('#entitlement-patch-editor'); });
...
$('#entitlement-patch-form').addEventListener('submit', async (event) => {
  ...
  try {
    await withBusy(form, async () => {
      ...
      toast('权益已更新');
      hideEditor('#entitlement-patch-editor');
      await loadEntitlements();
    }, '保存中…');
  } catch (error) { toast(error.message, 'danger'); }
});
...
async function pagedFetch(key, path, params = {}) {
...
function bindFilters(entries) {
  entries.forEach(([selector, key]) => {
    const node = $(selector);
    const loader = PAGED_LOADERS[key];
...
function resetFilters(selectors, key) {
...
const PURGE_SPECS = {
  ...
  'device-release-events': {
    ...
    reload: () => loadReleaseEvents(),
  },
};

let purgeResolver = null;
const purgeDialog = $('#purge-dialog');

// 优惠码「查看核销记录」跳过来时带的筛选；空表示看全部
let redemptionFilter = null;

// 所有批量清理走同一个弹窗：影响面 + 必填的天数，缺一不可。
function askPurge({ title, message, impact }) {
...
async function loadLicenseSessions() {
...
async function loadRedemptions() {
  ...
  try {
    data = await pagedFetch(
      'coupon-redemptions',
      '/coupon-redemptions',
      redemptionFilter ? { coupon_id: redemptionFilter.couponId } : {},
    );
  } catch (error) {
    toast(error.message, 'danger');
    return;
  ...
  chip.hidden = !redemptionFilter;
  if (redemptionFilter) chip.textContent = `仅看 ${redemptionFilter.couponCode}（点击清除）`;
...
async function loadDiagnostics() {
  ...
  await Promise.all(DIAGNOSTIC_KEYS.map(key => PAGED_LOADERS[key]()));
}
...
document.addEventListener('click', async (event) => {
  ...
  if (retry) {
    const loader = PAGED_LOADERS[retry.dataset.retry];
...
$('#panel-diagnostics').addEventListener('click', async (event) => {
  ...
  const voidRow = event.target.closest('[data-redemption-void]');  if (voidRow) {
    ...
    try {
      ...
      await Promise.all([loadRedemptions(), loadCoupons()]);
    } catch (error) { toast(error.message, 'danger'); }
    return;
  }
});
