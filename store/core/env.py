...
把项目根目录下 ``.env`` 里的键值对注入 ``os.environ``，存放 SMTP 授权码、支付宝私钥
这类**不该进版本库**的本地配置。优先级刻意保持「真实环境变量 > .env」：已存在的环境
变量不会被覆盖，所以 ``start.py`` 注入的本地默认值不会被 ``.env`` 抢掉。
"""
...
def load_dotenv(path: Path | None = None, *, override: bool = False) -> dict[str, str]:
    ...
    ``override=False``（默认）表示已有环境变量优先：容器/CI 里的真实变量应压过本地文件。
    """
    global _loaded
