...
def mask_secret(value: str | None) -> str:
    ...
    留末 4 位是为了让运营能确认「库里存的是我以为的那把新密钥」—— 全遮成 ``****``
    的话，密钥换没换成功在界面上完全看不出来，而这是配置页最常见的一次操作。

    末 4 位必须取自**密钥本体**（PEM 去掉头尾之后的 base64），不能直接取原串尾部：
    PEM 的最后几个字符永远是 ``-----``，直接截取的话每一把私钥都会显示成
    ``••••----``，看着像「换了」其实什么都没变 —— 打码值就彻底失去意义了。
    """
    text = (value or "").strip()
    if not text:
        return ""
