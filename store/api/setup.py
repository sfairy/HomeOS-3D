...
def setup_admin(
    ...
    # 邮箱形态校验先于任何状态判断：它只看请求体，与「有没有管理员」「引导密钥对不对」都无关，因此
    # 不会泄漏信息、也不会被当探针。必须校验是因为这个值是**管理员账号的登录标识**，而同一个值在其它
    # 入口（后台建账号、测试发信收件人）都是先过 ``is_valid_email`` 再落库的 —— 只有「首次设置」不过，
    # 而它恰恰是部署者最认真输入的一次。拼错的后果不是报错，而是带着漏了 @ 或后缀的登录标识一路跑下去。
    ...
    # 顺序有讲究，三层各自解决一件事：
    #   1. 「是否已有管理员」—— 最便宜的判空，且决定端点是否还开门。必须排在守卫之前：初始化成功后
    #      守卫会作废引导密钥，若先过守卫会得到「缺少引导密钥」这种摸不着头脑的 403，而正确答案是 409。
    #   2. 守卫 —— 仍要排在 argon2 之前：``hash_password`` 很贵，让未授权请求先烧一遍等于免费打满 CPU。
