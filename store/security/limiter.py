...
商店是自托管的单进程部署，进程内计数已经够用；失败模式简单且安全 —— 进程重启即清零，
只会临时放宽，不会因为限流组件本身不可用而把正常用户挡在门外；也不新增部署依赖。

与 ``reconcile._allow_query`` 的分工：那里按**订单号**节流，这里按**来源/主体**配额，
两者互补。若将来改成多 worker / 多实例，计数会按进程分裂（实际额度变成上限 × 进程数），
届时需要换成共享存储。
"""
