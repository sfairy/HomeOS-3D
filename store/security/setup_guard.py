...
``POST /store/v1/setup/admin`` 刻意不要求身份（首次设置时还没有账号可登），唯一闸门是「库里是否已有
...
#: 为什么**不**在密钥文件里写注释来标记：``cat $APP_DATA_DIR/setup-token`` 是 README 教给运营的取用
#: 方式，多出注释会让人拿到「注释 + 密钥」而在表单里粘出一个 403。密钥文件因此保持「一行就是一枚
#: 密钥」，出处另放一个文件。主应用有同构的一份，两处必须一起改。
...
class SetupGuard:
    ...
    def _is_ours(self, token: str) -> bool:
        ...
        证据 = 标记文件里的指纹与该令牌的指纹一致。只看「标记文件在不在」不够：运营完全可能把它换成
        自己的（或删掉重放一份），此时标记还在而密钥是运营的 —— 指纹对不上就不会被误删。读不到就当
        「不是我们的」：宁可留下没用的文件，也不删掉可能是运营预置的恢复手段。
        """
    ...
    def has_setup_privilege(self, request: Request, setup_token: str = "") -> bool:
        ...
        判据与 :meth:`authorize` 完全一致：本机直连，或带了正确的引导密钥。刻意共用这一份实现而不是
        各写一套 —— 两套判据一旦漂移，结果就是「状态页说可以初始化，真点下去却 403」；``/setup/status``
        用它决定能不能如实回答尚未初始化。
        """
