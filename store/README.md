...
- **确认吊销**（403 命中吊销短语，服务端已明确停用）→ 保持拦截，本地授权已被清空，状态 `REVOKED`。
...
下面的变量都可以写进项目根目录的 `.env`（见 `.env.example`，已 gitignore）。
...
2. **滚动容器的 `padding-top` 必须是 0。** 表头会吸附在「滚动容器内容盒顶部」，也就是
   ...
   `.admin-panel.active` 承担，`.admin-main` 只留左右与底部内边距。
...
`overflow: clip`，「操作」列会被裁掉且无法滚到——这是功能问题，不是观感问题。所以
`max-width: 1420px` 起让 `.table-wrap` 横向滚动。**代价是表头吸顶失效**：横向滚动容器同时
也是 `.table-wrap`，`sticky` 的参照物变成了这个不纵向滚动的容器（见 10.2）。选「所有列都够
得着」优先。
...
顺带把侧栏从 11 个平铺条目改成「运营 / 商品与营销 / 授权与用户 / 系统」四组（`.nav-group`），
每项配一个 17px 内联 SVG 图标（`currentColor` 描边，跟随文字色变化），选中态加左侧琥珀指示条
（`::before`）。
