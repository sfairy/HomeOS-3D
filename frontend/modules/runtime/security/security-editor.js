...
} from "../presence/presence-motion.js?v=2609260900";
import { mountInteraction3d } from "../core/runtime.js?v=2609260900";
import { openPresenceEditor } from "../presence/presence-editor.js?v=2609260900";
// 人物方案表（traveler / bean / glow）与 3D 侧 createWalker 用的是同一份：批量设置里的
// 「人物方案」选项与展示文案都从这里取，绝不另抄一份。
import { DESIGNS } from "../presence/presence-character.js?v=2609260900";
// 跨域批量应用对话框：config-editor 与本编辑器共用同一实现（本文件只提供字段描述表与目标集合）。
import { copyBatchFields, openBatchApply } from "../editor/batch-apply.js?v=2609260900";
import {
  EDITOR_SAVE_STATUS,
  serializeEditorDraft
} from "../core/editor-save-status.js?v=2609260900";
import { applyMdiMask } from "../core/static-helpers.js?v=2609260900";
// 门锁状态与槽位顺序从 lock-state.js 取（它本身只是运行侧的薄转出口）：面板上的「当前状态」
// 与舞台动画必须同一口径，所以绝不在本文件里重写状态映射。
import { LOCK_ENTITY_FIELDS, lockState } from "./lock-state.js?v=2609260900";
import {
  confirmAction,
  createDomFactory,
...
export async function openSecurityEditor({
  ...
  securityStyleLinkElement.href =
    "/api/v1/modules/interaction3d/security/security-editor.css?v=2609260900";
