...
import { COVER_DEFAULT_PREVIEW_POSITION } from "../core/static-helpers.js?v=2609260929";
// 系统「减少动态效果」偏好的唯一判定（实现见 core/motion-preference.js）：命中时姿态直接到位、
// 不做 420ms 插值。垂帘与卷帘共用同一个判定，保证两种帘型在无障碍设置下表现一致。
import { prefersReducedMotionNow } from "../core/motion-preference.js?v=2609260929";
...
const {
  ...
  ? import(
      new URL(
        "../../../static/3d-studio/loaders/studio-curtain-track.js?v=2609260929",
        import.meta.url
      )
    )
  : import("/static/3d-studio/loaders/studio-curtain-track.js?v=2609260929"));
