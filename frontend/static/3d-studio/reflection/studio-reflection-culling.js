 ...
 * 地面反射的可见性剔除。
 *
 * 位置：studio-ground-reflections.js 逐面镜子渲染反射贴图时调用本模块，
 *   把「不可能出现在镜像画面里的网格」临时隐藏，减少反射通道的绘制量。
 * 对外：createReflectionCulling 工厂，返回 {reset, add, begin, restore, stats}。
 * 工作方式：先 add 收集候选网格，再 begin(capture, camera) 以「该面镜子在镜像贴图里
 *   实际占用的 UV 范围」收窄相机投影矩阵，最后用收窄后的视锥做一次整体剔除，
 *   restore 把所有被隐藏的网格恢复可见。
 * 约定：capture.matrix 是「世界坐标 → 镜像贴图 UV 空间」的矩阵，
 *   capture.map 是反射渲染目标（用其宽度换算纹素尺寸）；本模块不申请任何 GPU 资源。
 */
...
export function createReflectionCulling(THREE) {
   ...
   * 针对一面镜子开始一轮剔除。
   *
   * 先把镜面源物体的世界包围盒投影到镜像贴图 UV 空间，得到它实际占用的矩形，
