 ...
 * 运行时家具网格合并。
 *
 * 加载完户型与家具、进入交互渲染前调用一次，把「同类型家具里材质参数一致」的网格合并成
 * 一个，降低绘制批次。合并会把颜色 / 粗糙度 / 金属度烘进顶点属性，因此结果必须换成走顶点
 * 属性的材质，带贴图的材质不参与。合并后统一换算到根节点局部坐标系并挂在 root 下。
 */
  ...
  for (const { item: item, group: itemGroup } of furnitureEntries) {
    ...
    if (!!RUNTIME_FURNITURE_TYPES.has(item.type) && itemGroup.parent === root) {
      itemGroup.traverse(node => {
        ...
        // 逐条排除不安全的情况：实例化或带子节点的网格可见性语义复杂；缺 position/normal 或做过
        // 局部绘制范围裁剪的几何合并后会错位；负行列式（镜像缩放）会翻转绕序；含贴图无法烘顶点。
        if (
          !node.isMesh ||
