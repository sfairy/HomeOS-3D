...
/**
 * 按连通分量放大树叶几何。
 * 只在 foliage 分组内建并查集，把共享顶点与「位置相同但被拆开」的顶点并到同一簇（导入模型常因
 * UV / 材质拆点，只按索引相连会把一片叶子拆成多块、放大后互相穿插）；每簇取包围盒中心，绕 Y 轴
 * 旋转 (簇序号 % 3 - 1) × 60°，再以簇中心为原点把顶点相对坐标乘以 scale。
 * 结果写在几何副本上，原几何与传入法线都不改动。
 */
