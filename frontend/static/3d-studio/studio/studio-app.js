...
import {
  ...
  addRollerCurtain,
  addTrackCurtain
} from "../loaders/studio-curtain-track.js?v=2609252210";
import { drawTelevisionPoster } from "../materials/studio-television-poster.js?v=2609252210";
import { apiAuthChallenge, apiRequestError } from "../../utils/api-request.js?v=2609252210";
...
import {
  ...
  positionPointMenu
} from "../../shared/menu-positioning.js?v=2609252210";
import { paletteColor } from "../../utils/colors.js?v=2609252210";
import { roundToDecimals } from "../../utils/numbers.js?v=2609252210";
// 未绑定窗帘的默认开合度：与运行时的未绑定兜底同值，只定义在 utils/cover-features.js 一处。
import { COVER_DEFAULT_PREVIEW_POSITION } from "../../utils/cover-features.js?v=2609252210";
import { yieldToIdle, yieldToScheduler } from "./studio-yield.js?v=2609252210";
...
import { buildItemBody, finishItemModel } from "./item-builders/registry.js?v=2609252210";
import {
...
} from "../materials/studio-surface-textures.js?v=2609252210";
import { createOverviewStack } from "./studio-overview-stack.js?v=2609252210";
import { windowGeometryParts } from "../plan/studio-window-geometry.js?v=2609252210";
import {
  MAX_CAMERA_POLAR_ANGLE,
  constrainCameraPosition,
  constrainCameraPose
} from "./studio-camera-constraints.js?v=2609252210";
import { addSecurityModel } from "../loaders/studio-security-models.js?v=2609252210";
import { createFloorTransition } from "./studio-floor-transition.js?v=2609252210";
import { floorOpeningPolygon } from "../plan/studio-floor-openings.js?v=2609252210";
import { createGroundReflections } from "../reflection/studio-ground-reflections.js?v=2609252210";
import { createMotionPresentation } from "./studio-motion-presentation.js?v=2609252210";
import { renderStudioAssetPalette } from "./studio-asset-palette.js?v=2609252210";
import {
  createWallSideMaterial,
  setWallGradientHeight,
  setWallCornerDistances
} from "../materials/studio-wall-materials.js?v=2609252210";
import {
  WARM_HOME_STYLE,
  WARM_WOOD_STYLE,
  applyItemFinish,
  decorateWarmFloor
} from "./studio-scene-style.js?v=2609252210";
...
import {
  ...
  applyMaterialStyle
} from "./studio-material-styles.js?v=2609252210";
import { createWarmTelevisionGlass } from "../materials/studio-television-glass.js?v=2609252210";
import {
  ...
  sha256,
  stableCacheJSON
} from "../../bridge/render-cache.js?v=2609252210";
import { transformSceneCamera } from "../../bridge/scene-frame.js?v=2609252210";
import { sceneUpdatePlan } from "../../bridge/scene-update.js?v=2609252210";
import { createDemandFrameLoop } from "../../bridge/frame-loop.js?v=2609252210";
import { cacheObjectTransforms } from "../../bridge/scene-matrices.js?v=2609252210";
...
import { debugLog } from "../../utils/debug-log.js?v=2609252210";
// 首屏分段埋点（`[3D-load]`）：开关与出口都在 utils/debug-log.js，本文件只负责在链上打点。
import { createLoadTiming } from "../../bridge/load-timing.js?v=2609252210";
// 舞台页的提前起跑（舞台页里非 null）：它已经替我们读过场景持久缓存、发过场景接口请求、
// 开始下载 stage 模块 —— 三件事都直接复用，绝不再做第二遍。
import { stageStartup } from "../stage-startup.js?v=2609252210";
// 场景持久缓存：把「归一后的场景文档」跨会话存起来，舞台页二次打开时跳过整段逐层归一。
import { prepareSceneDocument } from "../scene-persistent-cache.js?v=2609252210";
// 接口请求的超时预算由 utils/api-fetch.js 统一持有（requestStudioApi 是唯一出入口）。
import { apiFetch } from "../../utils/api-fetch.js?v=2609252210";
import * as threeModuleMin from "/static/vendor/three/0.186.0/three.module.min.js";
import { OrbitControls } from "/static/vendor/three/0.186.0/OrbitControls.js?v=2609252210";
...
import { GLTFLoader } from "/static/vendor/three/0.186.0/GLTFLoader.js?v=2609252210";
import { SameOriginDRACOLoader } from "../export/draco-loader.js?v=2609252210";
import {
  ...
  mapLightEffectState,
  lightEffectColorHex
} from "../../bridge/light-motion.js?v=2609252210";
import {
  ...
  wallJoinExtensions,
  wallSolidPieces
} from "../plan/geometry.js?v=2609252210";
import {
...
} from "../export/export-utils.js?v=2609252210";
// 布局层（折叠 / 拖拽调宽 / 状态记忆）与折叠快捷键都在 shared/ 下，与 /index 编辑器共用同一份
// 实现：两页的三栏骨架、分隔条交互、状态记忆是同一套需求，各写一份必然漂移。
import {
  createLayoutController,
  bindLayoutControls
} from "../../shared/layout-shell.js?v=2609252210";
import { bindLayoutShortcuts } from "../../shared/layout-shortcuts.js?v=2609252210";
import {
...
} from "../export/export-presets.js?v=2609252210";
import { reorderFloors } from "../plan/floor-order.js?v=2609252210";
import { syncControlValue } from "./ui-controls.js?v=2609252210";
import {
  initializeNumberInputs,
  initializeStudioSelects,
  syncStudioSelect
} from "./studio-widgets.js?v=2609252210";
import {
  createExternalModelManager,
  ALL_ITEM_MODELS
} from "../loaders/studio-external-models.js?v=2609252210";
import {
  createPlanDrawingTools,
  drawTrackedText
} from "../plan/studio-plan-drawing.js?v=2609252210";
import { createSpotShadowAtlasController } from "./studio-shadow-atlas.js?v=2609252210";
import { createRegionLightController, REGION_LIGHT_LAYER } from "../plan/studio-plan2-region-lights.js?v=2609252210";
import { createContactShadowController } from "../plan/studio-plan2-contact-shadows.js?v=2609252210";
import {
  ...
  normalizeLabelText,
  normalizePoint
} from "../loaders/studio-normalization.js?v=2609252210";
/**
...
const dracoLoader = new SameOriginDRACOLoader(
  "/static/3d-studio/export/draco-decoder-worker.js?v=2609252210"
);
  ...
  try {
    ...
    if (isStageViewerMode) {
      ...
      const { mountStage: mountStage } = await (stageStartup?.module ||
        import("/api/v1/modules/interaction3d/core/stage.js?v=2609252210"));
