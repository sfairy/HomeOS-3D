 ...
 */
import { resolvePageBehavior } from "./page-behavior.js?v=2609260946";
import {
  performanceWarnings,
  confirmPerformanceWarning
} from "./performance-warning.js?v=2609260946";
import { normalizeGroundReflection } from "./reflection-settings.js?v=2609260946";
import { apiErrorMessage } from "../utils/api-error.js?v=2609260946";
...
import { INTERACTION_PAGE_OPTIONS } from "../utils/interaction-pages.js?v=2609260946";
import { createDomFactory } from "../shared/dom-factory.js?v=2609260946";
import {
  INTERACTION3D_LIGHTING_MODES,
  normalizeInteraction3dLightingMode,
  BACKGROUND_THEMES,
  normalizeBackgroundTheme
} from "./definition.js?v=2609260946";
...
async function requestInteraction3dScene() {
  ...
  return withRequestTimeout(SCENE_REQUEST_TIMEOUT_MS, async abortSignal => {
    ...
    if (!response.ok) {
      // 文案归一交给 utils/api-error.js（唯一实现）。
...
export function renderInteraction3dInspector(hostElement, targetComponent, editorOptions) {
  ...
  planRenderButton.addEventListener("click", async () => {
    ...
    try {
      ...
      const { openInteraction3dAppearanceEditor: openAppearanceEditor } =
        await import("/api/v1/modules/interaction3d/editor/config-editor.js?v=2609260946");
  ...
  configureSecurityButton.addEventListener("click", async () => {
    ...
    try {
      ...
      const { openSecurityEditor: openSecurityEditor } =
        await import("/api/v1/modules/interaction3d/security/security-editor.js?v=2609260946");
  ...
  if (
    ...
    waitInteraction3dEditorView(targetComponent.id)
      ...
      // 等 3D 侧视图就绪再同步楼层控件：失败无声即可，用户到 3D 端点开时会看到真实状态。
      .catch(() => {});
  }
  ...
  for (const [visibilityValue, visibilityLabel] of [
    ...
    // backgroundVisible 缺省视为「隐藏（透明）」，只有显式 true 才算显示，与渲染端默认一致。
    const isActiveVisibility = (properties.backgroundVisible === true) === visibilityValue;
