 ...
 *
 * 位置：编辑器每次收到项目保存 / 协作推送后调用，结果驱动 3D 场景的更新策略；
 *   本文件只做纯计算，不接触 three.js 对象。
 * 对外导出：sceneUpdatePlan。
 * 全局约定：判定是「内容级」的 —— 只影响编辑器视图状态、不影响渲染结果的字段
 *   被显式排除（见下方两张忽略表），否则切换正交视图、拖动预览面板宽度都会触发整场重建。
 * 副作用：无，纯函数。
 */
