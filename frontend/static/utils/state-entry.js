 ...
 *
 * **P12 收口（续）：3D 运行时树（`modules/interaction3d/`）里那 33 处内联剥壳也收了。**
 *   那棵树原先被记成「跨加载边界、闸刻意不伸进去」，复核后那条理由不成立 —— 真实的约束只是
 *   「运行侧的文件不能写裸 `/static/...` 的静态 import（舞台页能以 `file:` 打开）」，而树里
 *   早就有七处条件动态导入。于是这段导入收进纯转出口 `modules/interaction3d/static-helpers.js`
 *   （桥），其余 16 份文件写普通的 `from "./static-helpers.js?v=20260919214245"`；第 9 条闸的扫描范围随之
 *   合上。那 33 处的下游逐处过目，结论与上面 47 处同一口径（`?.` 取字段 / 真值判定 /
 *   自己再归一一次三类，均不可观测）。
 */
