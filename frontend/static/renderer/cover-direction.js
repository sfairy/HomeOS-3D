 ...
 * 以及**同物不同名**那一对（`registry` 与 `cover-runtime` 各一份私有副本）。死的那份是前者的遗留物
 * —— 方向改由控件属性承载后，没有人再读那个开关实体了（3D 运行时树另有自己的实体推导，见
 * `modules/interaction3d/runtime.js`，那属于另一份知识）。
 *
 * 契约（下表是唯一口径）：
 *
 ...
 *   | 其它任何值 / 缺字段 / 非对象 | `false` |
 *
 * 判定是**严格相等**，不做大小写归一：值由编辑器写死为 `"normal"` / `"reversed"`（见 `home.js` 的
 * `["normal", "reversed"].includes(...)` 守卫），设备侧不会产生别的写法。写成宽松判定（例如
 * `!== "normal"` 就算反转）会把「字段缺失」也当成反转，而那正是**大多数**控件的状态 —— 一改就会让
 * 所有窗帘的开合图标反过来。
 */
